...
class MultilingualTextPreprocessor:
    ...
    def extract_keywords(
        self, text: str, language: Optional[str] = None, use_masked: bool = True
    ) -> List[str]:
        """Extract top keywords from text, optionally masking PII first for explainability"""
        if language is None:
            language = self.detect_language(text)

        if use_masked and self.enable_pii_masking:
            text, _ = self.mask_pii(text)

        if language == "fa":
            try:
                if hasattr(self, "farsi_tokenizer"):
                    words = self.farsi_tokenizer.tokenize_words(text)
                else:
                    words = text.split()
            except:
                words = text.split()
